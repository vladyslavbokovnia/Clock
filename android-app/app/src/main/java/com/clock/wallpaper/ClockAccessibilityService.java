...
public class ClockAccessibilityService extends AccessibilityService {
 ...
 void show(){if(overlay!=null)return;boolean upside=getSharedPreferences("clock",0).getBoolean("wallpaperUpsideDown",false)&&!getSharedPreferences("clock",0).getBoolean("normalOrientation",false);ScreenHandsOverlay v=new ScreenHandsOverlay(this,upside);if(upside)v.setRotation(180);overlay=v;WindowManager.LayoutParams lp=new WindowManager.LayoutParams(-1,-1,WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY,WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE|WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE|WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN|WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS,PixelFormat.TRANSLUCENT);wm.addView(overlay,lp);}
