...
public class ScreenHandsOverlay extends View {
  ...
  @Override protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight(),cx=w/2f,cy=h/2f;now=Calendar.getInstance();float hour=((now.get(Calendar.HOUR_OF_DAY)%12)+now.get(Calendar.MINUTE)/60f)/12f*360f;float minute=(now.get(Calendar.MINUTE)+now.get(Calendar.SECOND)/60f)/60f*360f;hand(c,cx,cy,Math.min(w,h)*.18f,hour,Math.max(10,w*.018f),Color.rgb(255,218,86));hand(c,cx,cy,Math.min(w,h)*.30f,minute,Math.max(7,w*.012f),Color.rgb(255,240,150));float wx=w*.291f,wy=h*.454f;float week=((now.get(Calendar.DAY_OF_WEEK)-1)/7f)*360f;hand(c,wx,wy,Math.min(w,h)*.075f,week,Math.max(5,w*.009f),Color.rgb(255,225,105));p.setStyle(Paint.Style.FILL);p.setColor(Color.rgb(255,218,86));c.drawCircle(cx,cy,Math.max(12,w*.022f),p);p.setColor(Color.rgb(75,20,4));c.drawCircle(cx,cy,Math.max(5,w*.010f),p);}
}
