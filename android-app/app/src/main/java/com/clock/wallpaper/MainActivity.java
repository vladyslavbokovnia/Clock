...
public class MainActivity extends Activity {
  ...
  @Override public void onCreate(Bundle b){super.onCreate(b); dpm=(DevicePolicyManager)getSystemService(DEVICE_POLICY_SERVICE); admin=new ComponentName(this,ClockAdminReceiver.class); LinearLayout box=new LinearLayout(this); box.setOrientation(LinearLayout.VERTICAL); box.setPadding(32,32,32,24);
    ...
    Button overlayButton=new Button(this); overlayButton.setText("Разрешить автоматическую заставку поверх приложений"); overlayButton.setOnClickListener(v->{if(Build.VERSION.SDK_INT>=23)startActivity(new Intent(android.provider.Settings.ACTION_MANAGE_OVERLAY_PERMISSION,android.net.Uri.parse("package:"+getPackageName())));}); box.addView(overlayButton);
    Button adminButton=new Button(this); adminButton.setText(dpm.isAdminActive(admin)?"Выключение экрана разрешено":"Разрешить автоматическое выключение экрана (необязательно)"); adminButton.setOnClickListener(v->{Intent i=new Intent(DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN);i.putExtra(DevicePolicyManager.EXTRA_DEVICE_ADMIN,admin);i.putExtra(DevicePolicyManager.EXTRA_ADD_EXPLANATION,"Необходимо только для выключения экрана после показа заставки. PIN или пароль не включается.");startActivityForResult(i,42);}); box.addView(adminButton);
